...
import { apiPost, FeedbackIn, FeedbackOut } from '../lib/api';
...
export function Feedback() {
  ...
  const [submittedFeedback, setSubmittedFeedback] = useState<FeedbackOut | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiPost<FeedbackOut>('/feedback', payload);
  ...
  return (
    <>
      ...
      <section className="grid" style={{ gridTemplateColumns: '1fr', maxWidth: '600px', margin: '0' }}>
        {submittedFeedback && (
          <div className="panel" style={{ borderLeft: '4px solid var(--accent)', background: 'rgba(59, 130, 246, 0.1)', marginBottom: '1rem' }}>
            <h3 style={{ margin: '0 0 0.5rem 0' }}>✓ Feedback submitted successfully</h3>
            <div style={{ marginTop: '1.5rem' }}>
              <h4>Your Submitted Feedback</h4>
              <p><strong>Was this platform useful?</strong> {submittedFeedback.useful ? 'Yes' : 'No'}</p>
              <p><strong>Comments:</strong> "{submittedFeedback.note}"</p>
              {submittedFeedback.content_id && (
                <p><strong>Related Content ID:</strong> {submittedFeedback.content_id}</p>
              )}
