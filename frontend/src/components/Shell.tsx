...
export function Shell({children,tab,setTab,onLogout,user}:{children:React.ReactNode;tab:string;setTab:(x:string)=>void;onLogout:()=>void,user:any}) {
  ...
  const adminNav=[['Overview',Home],['Sources',BookOpen],['Content Review',ClipboardCheck],['Evidence Review',BookCheck],['Feedback',MessageSquare],['Audit History',Target],['Analytics',ChartNoAxesCombined]] as const;
