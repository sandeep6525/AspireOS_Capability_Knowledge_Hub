...
from ..models import Assessment, Skill, ContentItem, Feedback, LearningPlan, User

def get_skill_gaps_analytics(db: Session, tenant_id: str | None = None):
    ...
    if tenant_id and tenant_id != "public":
        q = q.join(User, Assessment.user_id == User.id).filter(User.tenant_id == tenant_id)

    results = q.group_by(Assessment.skill_id, Skill.name).order_by(func.avg(gap_expr).desc()).limit(5).all()
...
def get_content_analytics(db: Session, tenant_id: str | None = None):
    # Content is global for now, tenant filtering applies only to user-data
    results = (
...
def get_feedback_analytics(db: Session, tenant_id: str | None = None):
    q = db.query(Feedback)
    if tenant_id and tenant_id != "public":
        q = q.join(User, Feedback.user_id == User.id).filter(User.tenant_id == tenant_id)

    total = q.count()
    useful = q.filter(Feedback.useful == True).count()
...
def get_learning_plans_analytics(db: Session, tenant_id: str | None = None):
    q = db.query(LearningPlan).filter(LearningPlan.status == 'active')
    if tenant_id and tenant_id != "public":
        q = q.join(User, LearningPlan.user_id == User.id).filter(User.tenant_id == tenant_id)
